...
data class NestingResult(
    ...
    val elapsedMs: Long,
    // إصلاح: مساحة القطع المرصوصة فعلياً. افتراضياً = sourceArea × totalPlaced
    // (صحيحة فقط لو كل القطع نسخ متطابقة من شكل واحد — حالة محرك DXF البحت).
    // أي محرك بيرص قطعاً مختلفة الحجم (خزائن/مقاسات مخصصة) لازم يمرر القيمة
    // الحقيقية بنفسه بدل الاعتماد على الافتراضي.
    val placedArea: Double = sourceArea * totalPlaced
) {
    val boardArea: Double get() = boards.sumOf { it.width * it.height }
    val usedArea: Double get() = placedArea
...
// إصلاح: مراحل واضحة لعملية الرص عشان الواجهة تقدر تعرض وصف حقيقي لكل مرحلة
// بدل شريط تحميل واحد صامت من غير أي سياق.
enum class NestingPhase { PLACING, OPTIMIZING, ABSORBING, PREPARING_PREVIEW }
...
data class NestingProgress(
    ...
    val boardIndex: Int,
    val percent: Int,
    val phase: NestingPhase = NestingPhase.PLACING
)
...
object NestingEngine {
    ...
    private const val MAX_SOURCE_ANCHORS = 96
    private const val MAX_TARGET_ANCHORS = 96
    private const val MAX_CANDIDATES_PER_ROTATION = 6000
    private const val MAX_ANGLE_SEEDS = 3
    private const val MAX_COARSE_ROTATIONS = 36
    ...
    // إصلاح أداء: حساب الشكل الفعلي (بعد الدوران والإزاحة) لقطعة واحدة، مرة
    // واحدة بس وقت ما بتتحط، بدل ما تتحسب من الصفر في كل مرة القطع المرصوصة
    // كلها بتتراجع (كانت بتحصل مرة لكل قطعة جديدة تتضاف = O(n²) على مستوى
    // اللوح الواحد بلا أي داعي، لأن القطع اللي اترصت قبل كده متغيّرتش).
    private fun cachePiece(p: NestingPiece): CachedPiece =
        CachedPiece(p, CachedPolygon(transformed(p.polygon.outer, p.rotationDeg, p.x, p.y), placedBounds(p)))
    ...
    fun nest(
        ...
        while (remaining > 0 && !isCancelled()) {
            ...
            // إصلاح أداء: بنراكم الـ Cache مع كل قطعة بدل ما نسيب findBestPlacement
            // يعيد بناءه بالكامل من الصفر مع كل قطعة جديدة تتضاف.
            val cachedPlaced = mutableListOf<CachedPiece>()
            ...
            while (remaining > 0 && !isCancelled()) {
                ...
                val best = findBestPlacement(shape, sourceAnchors, placed, config, { phase ->
                    ...
                    onProgress(NestingProgress(completed, copies, boardIndex, p.roundToInt().coerceIn(0, 99), NestingPhase.PLACING))
                }, isCancelled, cachedPlacedIn = cachedPlaced)

                if (best == null) break
                val finalPiece = best.copy(index = pieceIndex++)
                placed += finalPiece
                cachedPlaced += cachePiece(finalPiece)
                remaining--
                val done = copies - remaining
                onProgress(NestingProgress(done, copies, boardIndex, (done * 100 / copies).coerceIn(0, 100), NestingPhase.PLACING))
            }

            if (placed.isEmpty()) break

            // إصلاح: نُبلّغ الواجهة إننا دخلنا مرحلة "تحسين الرص" (تجربة عدة
            // استراتيجيات فرز لتقليل الهدر) قبل ما ندخل optimizeBoard، عشان
            // النص المعروض يوصف اللي فعلاً بيحصل بدل ما يفضل واقف على نفس الوصف.
            onProgress(NestingProgress(copies - remaining, copies, boardIndex, (((copies - remaining) * 100) / copies).coerceIn(0, 99), NestingPhase.OPTIMIZING))
            val optimized = optimizeBoard(placed, shape, sourceAnchors, config, isCancelled)
        ...
        if (totalPlaced == copies) {
            onProgress(NestingProgress(totalPlaced, copies, boards.lastOrNull()?.index ?: 1, 100, NestingPhase.PLACING))
        }
    ...
    private fun smartAbsorbLastBoard(
        ...
        for (piece in piecesToMove) {
            ...
            onProgress(NestingProgress(totalCopies, totalCopies, boards.size, pct, NestingPhase.ABSORBING))
        }
    ...
    private fun optimizeBoard(
        ...
        for (order in strategies) {
            ...
            val cachedRebuilt = mutableListOf<CachedPiece>()
            for (old in order) {
                if (isCancelled()) return bestResult
                val candidate = findBestPlacement(shape, sourceAnchors, rebuilt, config, {}, isCancelled, true, cachedPlacedIn = cachedRebuilt)
                if (candidate == null) break
                val finalPiece = candidate.copy(index = old.index)
                rebuilt += finalPiece
                cachedRebuilt += cachePiece(finalPiece)
            }
    ...
    private fun findBestPlacement(
        ...
        cachedPlacedIn: List<CachedPiece>? = null
    ): NestingPiece? {
        val rotations = allowedRotations(config)
        if (rotations.isEmpty()) return null

        // إصلاح أداء: لو المتصل بيمرر Cache جاهز (متراكم تدريجياً) بنستخدمه
        // زي ما هو، وإلا بس نبنيه من الصفر (توافقية مع أي نداء قديم).
        val cachedPlaced = cachedPlacedIn ?: placed.map { cachePiece(it) }
        ...
        // إصلاح أداء: نقاط الالتقاء (targets) بتعتمد بس على القطع المرصوصة
        // فعلاً — ملهاش أي علاقة بزاوية الدوران المُجرَّبة. كانت بتتحسب من
        // الصفر جوه bestForRotation في كل مرة تتنادى (حتى 81 مرة تقريباً لكل
        // قطعة واحدة: ~36 زاوية أولية + حتى 45 محاولة تنقيح)، فبنحسبها هنا
        // مرة واحدة بس ونمررها جاهزة لكل النداءات.
        ...
        for ((idx, rotation) in rotations.withIndex()) {
            if (isCancelled()) return null
            val candidate = bestForRotation(shape, sourceAnchors, targets, cachedPlaced, config, rotation, isCancelled)
            if (candidate != null) {
        ...
        val steps = doubleArrayOf(base / 2.0, base / 4.0, base / 8.0)
        ...
        for (step in steps) {
            for (seed in coarseBest) {
                for (k in -2..2) {
                    ...
                    val candidate = bestForRotation(shape, sourceAnchors, targets, cachedPlaced, config, angle, isCancelled)
                    if (candidate != null) {
    ...
    private fun placementScore(candidate: NestingPiece, placed: List<CachedPiece>, config: NestingConfig): Double {
        ...
        // تفضيل قوي جداً للارتفاع المنخفض + شكل أقرب للمستطيل
        val heightPenalty = envelopeH * 3_600_000.0
        val widthPenalty = envelopeW * 1_100_000.0
        val aspectPenalty = abs(envelopeW - envelopeH) * 180.0
        val bottomLeft = (b.minY - config.edgeTopMm) * 2400.0 + (b.minX - config.edgeLeftMm) * 1.3
        val contact = nearestPlacedDistance(candidate, placed)
        val contactPenalty = if (contact.isFinite()) contact * 18.0 else 0.0

        return heightPenalty + widthPenalty + aspectPenalty + bottomLeft + contactPenalty + (envelopeW * envelopeH * 1.8)
    }
